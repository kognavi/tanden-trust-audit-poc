...
```mermaid
flowchart LR
    E[Evidence JSON] --> V[JSON Schema validation]
...
実線は実装済みmoduleの責務を示しますが、単一のproduction applicationとして全経路が自動接続・deploy済みという意味ではありません。`AuditManager`はproviderと`PgSigningLogger`を接続しますが、Schema validationとEvidence Storeを内包しません。点線のWeb3 PoCはdigestを直接anchorし、署名済みであることをContractが検証しません。
...
|---|---|---|
| Structural validation | 実装済み。ただし全entry pointでの強制は未実装 | ingestion serviceでfail-closedに強制 |
