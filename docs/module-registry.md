...
| lib/signing-contract.js | signing contract constants, assertRawMessage, assertRawSignature, assertSecp256k1Key | Sign | Local/KMS共通のraw-message、SHA-256一回、secp256k1、IEEE P1363、KMS RAW/KeySpec contract | tests/signing-contract.test.js |
| lib/local-ecdsa-provider.js | SIGNATURE_ALGORITHM, LocalEcdsaProvider | Sign | local ECDSA secp256k1鍵によるraw-message/Evidence署名・検証provider | tests/local-ecdsa-provider.test.js, tests/cross-provider-parity.test.js |
| lib/aws-kms-provider.js | SIGNATURE_ALGORITHM, KMS_SIGNING_ALGORITHM, KMS_MESSAGE_TYPE, decodeDerSignatureToRaw, encodeRawToDer, AwsKmsProvider | Sign | AWS KMS ECC_SECG_P256K1 / ECDSA_SHA_256 / RAWによるraw-message/Evidence署名・検証provider | tests/aws-kms-provider.test.js, tests/cross-provider-parity.test.js |
| lib/metadata-signature.js | digest helpers, signSidecarMetadata, verifySidecarMetadataSignature | Sign | sidecar metadataのdigest生成とcanonical raw-message署名・検証、legacy provider fallback | tests/metadata-signature.test.js |
| lib/signature.js | constants, LocalEcdsaProvider, signature-digest exports, generateEcKeyPair, signRawMessage, verifyRawMessageSignature, deprecated aliases, signEvidence, verifyEvidenceSignature | Sign | LocalEcdsaProviderを使うEvidence署名API、明示的raw-message facade、digest helperの再export | tests/signature.test.js |
...
| lib/pg-signing-logger.js | PgSigningLogger, GENESIS_HASH | Ledger | PostgreSQLのappend-only signing event hash chainを記録・検証し、security gate向けに同一row snapshotから検証結果とimmutable eventを取得 | tests/pg-signing-logger.test.js |
| lib/internal-ledger-verifier.js | PgInternalLedgerVerifier, EVIDENCE_STORED_EVENT_TYPE | Ledger / Verification | full hash-chain検証後、指定された`evidence.stored` eventのEvidence IDと再計算digestを照合 | tests/internal-ledger-verifier.test.js |
