...
class PgInternalLedgerVerifier {
  constructor({ pgLogger } = {}) {
    if (
      !pgLogger ||
      typeof pgLogger.verifyChainIntegrityAndGetEvent !== "function"
    ) {
      throw new Error(
        "PgInternalLedgerVerifier requires a pgLogger implementing verifyChainIntegrityAndGetEvent()."
      );
    }
  ...
  async verifyRecordedEvidence({ ledgerEventId, evidenceId, digestHex } = {}) {
    ...
    const { integrity, event } =
      await this._pgLogger.verifyChainIntegrityAndGetEvent(ledgerEventId);
