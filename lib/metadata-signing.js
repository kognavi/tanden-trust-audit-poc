// ── lib/metadata-signing.js の先頭に追加 ──
// 役割: バリデーション済みメタデータから、署名対象ペイロード(署名フィールドを除いたもの)を構築する
// パイプライン位置: metadata.js(検証) → 2/3（ペイロード構築） → metadata-signature.js
...
async function createMetadataSigningPayload(metadata) {
  const validation = validateSidecarMetadataV2(metadata);
