...
class PgSigningLogger {
  ...
  async verifyChainIntegrity() {
    ...
    return verifySigningEventRows(result.rows);
  }

  /**
   * Verifies the full chain and selects an event from that exact query result.
   * This prevents a security gate from mixing chain and event reads from
   * different READ COMMITTED snapshots.
   */
  async verifyChainIntegrityAndGetEvent(eventId) {
    ...
    const integrity = await verifySigningEventRows(result.rows);
    const row = result.rows.find((candidate) => candidate.event_id === eventId);
    return {
      integrity,
      event: row === undefined ? null : mapSigningEventRow(row),
    };
  }
  ...
  async getLatestEvent() {
    ...
    return mapSigningEventRow(result.rows[0]);
  }

  /**
   * Reads one immutable ledger event by UUID without changing ledger state.
   * Callers using the row as a security precondition must use
   * verifyChainIntegrityAndGetEvent() instead.
   */
  async getEventById(eventId) {
    ...
    return result.rows.length === 0 ? null : mapSigningEventRow(result.rows[0]);
  }
}

async function verifySigningEventRows(rows) {
...
function assertValidEventId(eventId) {
  if (
    ...
    throw new Error("PgSigningLogger.getEventById requires a valid UUID eventId.");
  }
}

function mapSigningEventRow(row) {
  return {
    sequence: Number(row.sequence),
    eventId: row.event_id,
    eventType: row.event_type,
