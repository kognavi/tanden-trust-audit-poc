    ...
    try {
      await this.client.send(
        new PutObjectCommand({
          ...
          // Critical#3: WORM保証。同一キーが既に存在する場合はS3が
          // HTTP 412 (PreconditionFailed) を返し、書き込みを拒否する。
          IfNoneMatch: "*",
        })
      );
    } catch (error) {
      if (isPreconditionFailedError(error)) {
        throw createS3StoreError(
          `Object already exists and cannot be overwritten (WORM violation attempt): ${normalizedKey}`,
